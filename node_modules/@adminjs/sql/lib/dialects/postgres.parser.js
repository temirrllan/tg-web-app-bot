...
export class PostgresParser extends BaseDatabaseParser {
    ...
    async getResources(tables, schemaName) {
        const resources = await Promise.all(tables.map(async (tableName) => {
            try {
                const resourceMetadata = new ResourceMetadata(this.dialect, this.knex, this.connectionOptions.database, schemaName, tableName, await this.getProperties(tableName, schemaName));
