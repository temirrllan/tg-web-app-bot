  ...
  router.get('/api/stats', async (req, res) => {
    ...
    try {
      ...
      const [
        ...
        safe(`SELECT COUNT(*)::int AS val FROM users WHERE created_at AT TIME ZONE 'Asia/Almaty' >= (NOW() AT TIME ZONE 'Asia/Almaty')::date - INTERVAL '7 days'`),
        safe(`SELECT COUNT(*)::int AS val FROM users WHERE created_at AT TIME ZONE 'Asia/Almaty' >= (NOW() AT TIME ZONE 'Asia/Almaty')::date - INTERVAL '30 days'`),
        ...
        safe(`SELECT ROUND(COALESCE(AVG(cnt), 0)::numeric, 1)::float AS val FROM (SELECT COUNT(*)::numeric AS cnt FROM habits WHERE is_active = true GROUP BY user_id) t`, 0),
        safe(`SELECT ROUND(COALESCE(AVG(streak_current), 0)::numeric, 1)::float AS val FROM habits WHERE is_active = true AND streak_current > 0`, 0),
